...
xdebug_status ()
{
    ...
    if [[ $OS_TYPE == "MINGW" ]]; then
        winpty docker exec -it $PHP_FPM_CONTAINER sh -c 'php -v'
...
xdebug_start ()
{
    ...
    # If running on Windows, need to prepend with winpty :(
    if [[ $OS_TYPE == "MINGW" ]]; then
        winpty docker exec -it $PHP_FPM_CONTAINER sh -c "${ON_CMD}"
...
xdebug_stop ()
{
    ...
    if [[ $OS_TYPE == "MINGW" ]]; then
        ...
        # winpty docker exec -it laradock_php-fpm_1 sh -c 'bla bla bla'
        # Thanks to @michaelarnauts at https://github.com/docker/compose/issues/593
        winpty docker exec -it $PHP_FPM_CONTAINER sh -c "${OFF_CMD}"
        docker restart $PHP_FPM_CONTAINER
